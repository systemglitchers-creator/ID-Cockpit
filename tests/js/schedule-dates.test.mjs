...
// The plan is 585 sessions since the 2026-09-10 re-cut, so 25 read at Aug 8
// leaves 6 owed (555 unread into 554 slots + 5); it was 30 read under 590.
...
test("a slip no longer pushes the finish date out", () => {
  const app = behind(25);                       // 6 study days short
  const m = app.IDCockpit.compute();
...
test("the debt surfaces as sessions owed, not days lost", () => {
  const m = behind(25).IDCockpit.compute();
...
test("reading ahead dissolves the catch-up on its own", () => {
  assert.equal(behind(25).IDCockpit.compute().makeup, 6);
  assert.equal(behind(28).IDCockpit.compute().makeup, 3);
  assert.equal(behind(31).IDCockpit.compute().makeup, 0);
});
...
test("the first double does not recede as days pass", () => {
  ...
  assert.equal(on(SAT_AUG_8, 25), "Sun Aug 23 2026");
  assert.equal(on(new Date(2026, 7, 12, 9, 0, 0), 25), "Sun Aug 23 2026", "still Aug 23 four days later");
