...
function tonight(sections, progress, now) {
  ...
  // Pinned second readings (r.extra) sit on their own day and never queue.
  const rows = [], extras = [];
  let planEnd = 0;
  ...
  // ...but a reading pinned to today, or left over from an earlier day, is
  // still due — the point of the day is that both get read. Only the left-over
  // ones count as owed; a pinned day still to come is simply the plan.
  extras.forEach((r) => { if (r.gi <= k0) out.push(r); });
  if (!out.length) return null;
  const overdue = extras.filter((r) => r.gi < k0).length;
  return { sessions: out, makeup: (doubling ? rows.length - slots : 0) + overdue };
}
